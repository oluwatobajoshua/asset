...
<div class="view card card-primary card-outline">
  ...
  <div class="card-body table-responsive p-0">
    <table class="table table-hover text-nowrap">
        <tr>
            <th><?= __('User') ?></th>
            <td><?= $bidding->has('user') ? $this->Html->link($bidding->user->id, ['controller' => 'Users', 'action' => 'view', $bidding->user->id]) : '' ?></td>
        </tr>
        <tr>
            <th><?= __('Status') ?></th>
        ...
        <tr>
            <th><?= __('Remark') ?></th>
            <td><?= $this->Number->format($bidding->remark) ?></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td><?= $this->Number->format($bidding->reviewed_amount) ?></td>
        </tr>
        <tr>
            <th><?= __('Created') ?></th>
...
<div class="related related-permits view card">
  <div class="card-header d-sm-flex">
    <h3 class="card-title"><?= __('Related Permits') ?></h3>
    <div class="card-toolbox">
      <?= $this->Html->link(__('New'), ['controller' => 'Permits' , 'action' => 'add'], ['class' => 'btn btn-primary btn-sm']) ?>
      <?= $this->Html->link(__('List '), ['controller' => 'Permits' , 'action' => 'index'], ['class' => 'btn btn-primary btn-sm']) ?>
    </div>
  </div>
  <div class="card-body table-responsive p-0">
    <table class="table table-hover text-nowrap">
      ...
      <?php if (empty($bidding->permits)) { ?>
        <tr>
            <td colspan="6" class="text-muted">
              Permits record not found!
            </td>
        </tr>
      <?php }else{ ?>
        <?php foreach ($bidding->permits as $permits) : ?>
        <tr>
            <td><?= h($permits->id) ?></td>
            <td><?= h($permits->bidding_id) ?></td>
            <td><?= h($permits->receipt_no) ?></td>
            <td><?= h($permits->created) ?></td>
            <td><?= h($permits->modified) ?></td>
            <td class="actions">
              <?= $this->Html->link(__('View'), ['controller' => 'Permits', 'action' => 'view', $permits->id], ['class'=>'btn btn-xs btn-outline-primary']) ?>
              <?= $this->Html->link(__('Edit'), ['controller' => 'Permits', 'action' => 'edit', $permits->id], ['class'=>'btn btn-xs btn-outline-primary']) ?>
              <?= $this->Form->postLink(__('Delete'), ['controller' => 'Permits', 'action' => 'delete', $permits->id], ['class'=>'btn btn-xs btn-outline-danger', 'confirm' => __('Are you sure you want to delete # {0}?', $permits->id)]) ?>
            </td>
        </tr>
        <?php endforeach; ?>
      <?php } ?>
